<template>
    <div class="container">
        <h2 style="background-color: #1d68a7; color: #fff; font-family: Lucida Console" class="p-3 text-center">
            Нерешенные вопросы</h2>
        <div v-for="category in categories">
            <div class="card mb-3" v-for="question in questions"
                 v-if="category.id === question.category_id && question.status === 0">
                <div class="card-body">
                    <h5 class="card-title">Категория: {{ category.title }}</h5>
                    <h6 class="card-subtitle mb-2 text-muted">Отправитель: {{ question.sender_info }}</h6>
                    <p class="card-text">Вопрос: {{ question.question_text }}</p>
                    <a href="#" class="card-link">
                        <router-link class="btn"
                                     :to="{name: 'ShowQuestion', params: {questionId: question.id, flag: false}}">
